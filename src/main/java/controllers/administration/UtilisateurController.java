...
public class UtilisateurController implements Serializable {
    ...
    private List<OrclassIntermediaires> colIntermediaire;
...
//        RequestContext.getCurrentInstance().reset(":accord:formPersonnel");
    ...
    public void init() {
        ...
        colProfil = (List<OrclassProfils>) profilsDao.getAllProfilHaveAccesByEntreprise(entreprise);
        colProfil.add(chargerProfilAdmin());
        if (Objects.equals(entreprise.getLogin_user_automatique(), Boolean.TRUE)) {
    ...
    public void chargerUtilisateur() {
        if (entreprise != null && entreprise.getIdEntreprise() != null) {
            datasource = (List<OrclassUtilisateurs>) utilisateursDao.listUtilisateurWithFilters(entreprise);
        }
...
//            PrimeFaces.current().executeScript("PF('principal').select(0);");
    ...
    void initialiseSession() {
        ...
        colProfil = (List<OrclassProfils>) profilsDao.getAllProfilHaveAccesByEntreprise(entreprise);
        colProfil.add(chargerProfilAdmin());
        colIntermediaire = (List<OrclassIntermediaires>) intermediairesDao.listeIntermediaireByEntreprise(entreprise);
        selectedProfils = new ArrayList<>();
...
//        colProfil=new ArrayList<>();
    ...
    public String createLoginUser() {
        ...
        Long nbre = utilisateursDao.nbreUserCreateByCompagny(entreprise);
        nbre++;
        login = nbre.toString().length() == 1 ? "0" + nbre.toString() : nbre.toString();
        if (entreprise.getPrefix_login_user() != null && !"".equals(entreprise.getPrefix_login_user())) {
...
//            if (utilisateursDao.findEntityHavingValue("login", utilisateurs.getLogin().toUpperCase()) == null) {
            ...
            this.serviceEntreprise.addUtilisateur(utilisateurs, selectedProfils, entreprise);
//            }
...
//        user.setCle_securite(Crypto.sha256("141087"));// la cle de securite c est 6chiffre important lors de la mise a jour d un mot de passe
        ...
        OrclassEntreprises e = this.entreprise;
        if (cle != null) {
...
//        contenuMail = contenuMail + "\n CORDIALEMENT L EQUIPE E-SPHERE  " + "\n LIEN DE CONNECTION/CONNECTION LINK : " + cheminApplication();
    ...
    public String updateUser() {
        ...
        String entete[] = {LocaleHelper.getLocaleString(RecupBundle.FichierBundle, "utilisateur", null, myLoc)};
        String[] detail = {entete[0], "Parametres"};
        if (Objects.equals(utilisateurs.getAllAccessForIntermediaire(), Boolean.TRUE) && utilisateurs.getIdIntermediaire() != null && utilisateurs.getIdIntermediaire().getIdIntermediaire() != null) {
            PrimeFaces.current().dialog().showMessageDynamic(new FacesMessage("VERIFIER LA GESTION D ACCESS POUR L INTERMEDIAIRE ...PLEASE CHECK ONE OPTION FATAL ERROR "));
            return null;
...
//            }
    ...
    public List<OrclassIntermediaires> getColIntermediaire() {
        return colIntermediaire;
    }

    public void setColIntermediaire(List<OrclassIntermediaires> colIntermediaire) {
        this.colIntermediaire = colIntermediaire;
    }
