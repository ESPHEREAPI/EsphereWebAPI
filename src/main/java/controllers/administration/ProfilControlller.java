...
//        modulesSource = new ArrayList<>();
        ...
        if (Objects.equals(entreprise.getProfil_code_automatique(), Boolean.TRUE)) {
     ...
     public String codeprofil(){
      String code;
      Long nbre=profilsDao.nbreProfileCreateByCompagny(entreprise);
      nbre++;
      code=nbre.toString().length()==1 ? "0"+nbre.toString():nbre.toString();
         if (entreprise.getPrefix_code_profile()!=null && !"".equals(entreprise.getPrefix_code_profile())) {
            return entreprise.getPrefix_code_profile()+""+code;
         }
         return code;
     }
    ...
    public void getallProfilAccessByEntreprise() {
        if (entreprise != null && entreprise.getIdEntreprise() != null) {
            listeProfileAccesByEntreprise = profilsDao.getAllProfilHaveAccesByEntreprise(entreprise);
            for (OrclassProfils p : profilsDao.allProfilByEntrePrise(entreprise)) {
    ...
    public String saveProfilAccess() {
        ...
        try {
            ...
            if ((profilsByUser == null || profilsByUser.getIdProfil() == null) && profilsByUser.getCode() != null) {
                profilsByUser.setIdEntreprise(entreprise);
...
//                actionSource.addAll(menu1);
    ...
    public DualListModel<OrclassModules> getModulesModel() {
        ...
        if (entreprise != null && entreprise.getIdEntreprise() != null && (profilsByUser == null || profilsByUser.getIdProfil() == null)) {

            if (modulesModel != null && !modulesModel.getSource().isEmpty()) {
                modulesSource = new ArrayList<>();
                modulesTarget = new ArrayList<>();
                modul = securiteService.getAllModulesByEntreprise(entreprise);
    ...
    public void saveFonctionnnaliteByModule() {
        ...
        if (moduleSelectForTarget != null && moduleSelectForTarget.getIdModule() != null) {
            ...
            if (instanceTableau == 0) {
                int taille1 = securiteService.getAllModulesByEntreprise(entreprise).size();
            ...
            if (profilsByUser != null && profilsByUser.getIdProfil() != null) {
                ...
                for (OrclassFonctionnalites f : fonctionaliteModel.getSource()) {
                    listPa = profilsAccesDao.getAllProfilByFonctionnalite(f,profilsByUser, entreprise);
                    if (listPa.isEmpty()) {
                        continue;
                    }
