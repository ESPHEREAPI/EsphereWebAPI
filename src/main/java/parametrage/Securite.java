...
//                .setParameter("value", Boolean.TRUE);
    ...
    public void addUtilisateur(OrclassUtilisateurs u, List<OrclassProfils> profil, OrclassEntreprises e) {
...
//                user.setPassword(Crypto.sha256(u.getPassword().toUpperCase()));
                ...
                user.setIdIntermediaire(u.getIdIntermediaire());
...
//                user = em.merge(user);
    ...
    @Override
    public Collection<OrclassModules> getAllModulesByEntreprise(OrclassEntreprises e) {
        Query q;
        q = em.createQuery("SELECT em.orclassModules FROM OrclassEntreprisesModules em WHERE em.orclassEntreprises.idEntreprise= :idEntreprise")
    ...
    public String addProfilForAccess(Collection<OrclassModules> modules, OrclassFonctionnalites[][] f, OrclassProfils pr) {
        ...
        try {
            ...
            if (p == null) {
                ...
                p.setIdEntreprise(pr.getIdEntreprise());
                em.persist(p);
...
//               .setParameter("idProfilAcces", pa.getIdProfilAcces());
    ...
    @Override
    public String addRegistreProduction(OrclassRegistreProduction pr, OrclassCategories cat, OrclassEntreprises e) {
        String messageError = null;
        try {
            tx.begin();

            OrclassRegistreProduction production = null;
            OrclassElt_Categorie_Compagnie elts = null;
            production = productionDao.finKeyRegistreProductionByCompagnie(e, pr.getCode());
            if (production != null && production.getId() != null) {
                messageError = "FATAL ERROR PLEASE CHANGE CODE... PRODUCTION REGISTER EXISTS ";
                throw new GlobalException(messageError);
            }
            production = new OrclassRegistreProduction(pr.getCode(), pr.getPrefixe());
            production.setIdEntreprise(e);
            production.setNum_debut(pr.getNum_debut());
            production.setNum_fin(pr.getNum_fin());
            production.setPlage_numero(pr.getPlage_numero());
            em.persist(production);

            elts = elt_Categorie_CompagnieDao.finkey(cat, e);
//            if (elts != null && elts.getId() != null) {
//                messageError = "FATAL ERROR PLEASE ...PRODUCTION REGISTER EXISTS FOR CATEGORY ELEMENT TRY AGAINST ";
//                throw new GlobalException(messageError);
//            }
            if (elts != null) {
//                elts = new OrclassElt_Categorie_Compagnie();
//                elts.setIdCategories(cat);
//                elts.setIdEntreprise(e);
                elts.setIdRegistreProduction(production);
                em.merge(elts);
            ...
            try {
                tx.rollback();
                logger.error("Erreur Survenu", ex.getMessage());
                throw new GlobalException("GlobalException survenu " + ex.getMessage() + " " + messageError);
    ...
    @Override
    public void addPlafondMaladie(OrclassPlafondMaladie pm, List<OrclassRubriquePrestation> listSelectedRubriquePrestation, OrclassDetailPlafondMaladie dpm, OrclassEntreprises e) {
        String messageError = "FATAL ERROR PLEASE CHANGE CODE... VALUE  EXISTS ";
        try {
            tx.begin();

            OrclassPlafondMaladie plafondMaladie = null;
            OrclassRubriquePrestation rubriquePrestation = null;
            OrclassDetailPlafondMaladie detailPlafondMaladie = null;
            plafondMaladie = plafondMaladieDao.finkey(pm.getCode(), e);
            ...
            plafondMaladie.setIdEntreprise(e);
            plafondMaladie.setCode(pm.getCode());
            plafondMaladie.setLibelle(pm.getLibelle());
            plafondMaladie.setMontant(pm.getMontant());
            plafondMaladie.setModeControle(pm.getModeControle());
            plafondMaladie.setNombre_Acte(pm.getNombre_Acte());
            plafondMaladie.setPriode(pm.getPriode());
            plafondMaladie.setDomaine_application(pm.getDomaine_application());
            plafondMaladie.setDateCreation(new Date());
            plafondMaladie.setUniteDuree(pm.getUniteDuree());

            em.persist(plafondMaladie);
            for (OrclassRubriquePrestation rp : listSelectedRubriquePrestation) {
                detailPlafondMaladie = detailPlafondMaladieDao.finKey(pm, rp, e);
                if (detailPlafondMaladie != null && detailPlafondMaladie.getId() != null) {
                    continue;
                }
                detailPlafondMaladie = new OrclassDetailPlafondMaladie();
                detailPlafondMaladie.setIdEntreprise(e);
                detailPlafondMaladie.setIdPlafondMaladie(plafondMaladie);
                detailPlafondMaladie.setIdRubriquePrestation(rp);
                detailPlafondMaladie.setBarem(dpm.getBarem());
                detailPlafondMaladie.setCode_ss(dpm.getCode_ss());
                detailPlafondMaladie.setForfait(dpm.getForfait());
                detailPlafondMaladie.setModeCalculDetailMaladie(dpm.getModeCalculDetailMaladie());
                detailPlafondMaladie.setTaux(dpm.getTaux());
                detailPlafondMaladie.setPlafond(dpm.getPlafond());
                detailPlafondMaladie.setTypeDetailMaladie(dpm.getTypeDetailMaladie());
                detailPlafondMaladie.setDateCreation(new Date());
                em.persist(detailPlafondMaladie);
            }
    ...
    @Override
    public void addRegistreSinistre(OrclassRegistreSinistre re, OrclassBranches cat, OrclassEntreprises e, OrclassUtilisateurs user) throws GlobalException {
        try {
            tx.begin();

            OrclassRegistreSinistre rs = null;
            OrclassElt_Categorie_Compagnie ets = null;
            List<OrclassCategories> listeCategories = new ArrayList<>();
            rs = finKeyRegistreSinistre(re.getCode(), e, re.getIdExercice());
            if (rs != null && rs.getId() != null) {
                throw new GlobalException("VALUE IS EXISTS PLEASE TRY AGAINST...OR SEE ADMINISTION ");
            }
            if (rs == null) {
                rs = new OrclassRegistreSinistre();
                rs.setCode(re.getCode());
                rs.setIdEntreprise(e);
                rs.setIdExercice(re.getIdExercice());
                rs.setNum_Registre(BigInteger.ZERO);
                rs.setIdUtilisateur(user);
                em.persist(rs);
//                listeCategories = elt_Categorie_CompagnieDao.listeCategoriesNotHaveRegistreSinistre(e, cat);
//                if (listeCategories.isEmpty()) {
//                     throw new GlobalException("LIST IS EMPTY ...VALUE DO NOT CREAT.. CHANGE VALUE");
//                }
//                for (OrclassCategories c : listeCategories) {
//                    ets = elt_Categorie_CompagnieDao.finkey(c, e);
//                    if (ets != null && ets.getId() != null && ets.getIdRegistreSinistre()==null) {
//                        ets.setIdRegistreSinistre(rs);
//                        em.merge(ets);
//                    }else{
//                        ets=new OrclassElt_Categorie_Compagnie();
//                        ets.setIdRegistreProduction(null);
//                        ets.setMax(0);
//                        ets.setMin(0);
//                        ets.setMin_garantie(0);
//                        ets.setIdCategories(c);
//                        ets.setIdEntreprise(e);
//                        ets.setIdRegistreSinistre(rs);
//                        em.persist(ets);
//                    }
//                }

//                if (ets == null || rs.getId() == null) {
//                    throw new GlobalException("ELEMNET CATEGORIE INEXISTANT...PLEASE TRY AGAINST");
//                }
            }

            tx.commit();

        } catch (NotSupportedException | HeuristicRollbackException | HeuristicMixedException | RollbackException | IllegalStateException | SecurityException | SystemException excep) {
            try {
                tx.rollback();
                logger.error("Erreur Survenu", excep.getMessage());
//                throw new GlobalException("GlobalException survenu " + ex.getMessage() + " " + ex.getCause().getMessage());

            } catch (GlobalException egx) {

                logger.error("erreur survenu", egx.getMessage() + " " + egx.getCause().getMessage());
    ...
    // recuperation de la dernier ligne du numero de registre pour une categorie precise
    public BigInteger lastNumRegistreSinistre(OrclassCategories cat, OrclassEntreprises e, OrclassExercice ex) {
        Query q;
        OrclassRegistreSinistre rs;
        BigInteger num = BigInteger.ZERO;
        q = em.createQuery(" select r.num_Registre FROM OrclassRegistreSinistre r WHERE r.id= (SELECT MAX(rs.id) from OrclassRegistreSinistre rs  JOIN rs.orclassElt_Categorie_CompagnieList ets where  ets.idCategories= :c and   rs.idExercice.Code= :code and rs.idEntreprise= :e )")
                .setParameter("c", cat)
                .setParameter("e", e)
        ...
        rs = (OrclassRegistreSinistre) q.getResultList().toArray()[0];
        if (rs != null && rs.getId() != null) {
            num = rs.getNum_Registre().add(BigInteger.ONE);
        }
        return num;
    }

    public OrclassRegistreSinistre finKey(OrclassCategories cat, OrclassEntreprises e, OrclassExercice ex, BigInteger numRegistre) {
        Query q;
        q = em.createQuery("SELECT r from OrclassRegistreSinistre r JOIN r.orclassElt_Categorie_CompagnieList elts where elts.idCategories= :cat and r.num_Registre= :num and r.idEntreprise= :e and r.idExercice= :ex ")
    ...
    public OrclassRegistreSinistre finKeyRegistreSinistre(OrclassCategories cat, OrclassEntreprises e, OrclassExercice ex) {
        Query q;
        q = em.createQuery("SELECT r from OrclassRegistreSinistre r JOIN r.orclassElt_Categorie_CompagnieList elts where elts.idCategories= :cat  and r.idEntreprise= :e and r.idExercice= :ex ")
                .setParameter("cat", cat)
                .setParameter("e", e)
                //                 .setParameter("num", numRegistre)
                .setParameter("ex", ex);
    ...
    public String createCleSecuriteForUser(OrclassEntreprises e) {
//        user.setCle_securite(Crypto.sha256("141087"));// la cle de securite c est 6chiffre important lors de la mise a jour d un mot de passe
    ...
    public String updateCleSecuriteForUser(OrclassEntreprises e, OrclassUtilisateurs u) {
//        user.setCle_securite(Crypto.sha256("141087"));// la cle de securite c est 6chiffre important lors de la mise a jour d un mot de passe

        String cle = KeyGenCode.gen();
        if (cle != null && u != null && u.getIdEntreprise() != null) {
            //verification si cette cle a ete deja generer  
    ...
    @Override
    public void resetCleSecurite(OrclassUtilisateurs u, OrclassEntreprises e) {
        if (u != null && u.getIdUtilisateur() != null && e != null && e.getIdEntreprise() != null) {
