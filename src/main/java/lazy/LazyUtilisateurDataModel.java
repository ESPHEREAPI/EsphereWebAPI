...
public class LazyUtilisateurDataModel extends LazyDataModel<OrclassUtilisateurs> {
    ...
    public LazyUtilisateurDataModel(List<OrclassUtilisateurs> datasource, OrclassEntreprises entreprises) {
        this.datasource = datasource;
        this.entreprises=entreprises;
    }

    public LazyUtilisateurDataModel(OrclassEntreprises entreprises) {
        this.entreprises = entreprises;
        this.datasource=(List<OrclassUtilisateurs>) utilisateursDao.listUtilisateurWithFilters( entreprises);
    }
    ...
    public List<OrclassUtilisateurs> load(int offset, int pageSize, Map<String, SortMeta> sortBy, Map<String, FilterMeta> filterBy) {
       String filterValue = null;
        ...
        if (filterValue == null || filterValue.trim().equals("")) {
            ...
            datasource = (List<OrclassUtilisateurs>) utilisateursDao.listUtilisateurWithFilters(offset, offset, filterValue, entreprises);
            setRowCount(utilisateursDao.countUtilisateurWithFilters(filterValue, entreprises).intValue());
        }
