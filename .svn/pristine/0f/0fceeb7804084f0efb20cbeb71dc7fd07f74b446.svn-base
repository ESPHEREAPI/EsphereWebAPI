...
public class EntrepriseController implements Serializable {
    ...
    private Collection<OrclassEntreprises> currentEntreprise = new ArrayList<>();
    Date debut, fin;
...
//        RequestContext.getCurrentInstance().reset(":form1");
    ...
    void initialiseSession() {
        ...
        currentEntreprise.add(entreprise);
        entreprise = new OrclassEntreprises();
...
//      this.doRedirect(chemin);
    ...
    public String updateEntreprise() {
        ...
        try {
            ...
            if (entreprise.getIdEntreprise() != null) {

                entrepriseDao.update(entreprise);
                String[] detail = {entete[0], entreprise.getIdEntreprise().toString()};
                summary = LocaleHelper.getLocaleString(RecupBundle.FichierBundle, "summary.update.succes", entete, myLoc);
                msgdetail = LocaleHelper.getLocaleString(RecupBundle.FichierBundle, "detail.update.succes", detail, myLoc);
                ctx.getExternalContext().getSessionMap().put(GlobalFonctions.ENTREPRISE_ACTIF, entreprise);
    ...
    public String showDetails(OrclassEntreprises item) {
        if ((entreprise == null || entreprise.getIdEntreprise() == null) && (item != null && item.getIdEntreprise() != null)) {

            this.setEntreprise(item);

//            this.setAbsolutePathImages(item.getChemin_logo());
        }
//        else {
        if (entreprise.getChemin_logo() != null && entreprise.getImage() != null) {
            this.setAbsolutePathImages(entreprise.getChemin_logo());
            File file = new File(this.getAbsolutePathImages());
            if (file.exists() == false) {
                GlobalFonctions.createImageByLibellePhotos(entreprise.getChemin_logo(), entreprise.getImage());
            }
...
//        }
    ...
    public Collection<OrclassEntreprises> getCurrentEntreprise() {
        return currentEntreprise;
    }

    public void setCurrentEntreprise(Collection<OrclassEntreprises> CurrentEntreprise) {
